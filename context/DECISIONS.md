...
## 2026-09-17 — D025: The UNet control is trained twice — matched lr 5e-5 and from-scratch lr 1e-3

**Context.** `unet_small.yaml` inherits every hyperparameter from `tiny_tl.yaml`, including
AdamW `lr 5e-5`. That is a *fine-tuning* rate chosen for pretrained weights; a randomly
initialised network has 15 batches × 50 epochs = 750 steps to travel from random to useful, and at
5e-5 it is likely to be undertrained, making the control a strawman. ROADMAP §11 already requires
"two serious attempts" before "tiny doesn't beat UNet" is published, so the project's own standard
is that the control must be fair before it counts.

**Decision (Ameya, 2026-09-17).** Two control runs, both reported:
`unet_small.yaml` (lr 5e-5, the strictly matched recipe — every hyperparameter identical to
tiny-TL) and `unet_small_lr1e-3.yaml` (lr 1e-3, the conventional from-scratch AdamW rate — the
single deliberate difference, marked `# CONTROL-B`). The pretraining question (ROADMAP §10 goal 3)
is answered against the *better* of the two; both rows appear in `RESULTS.md`.

**Alternatives rejected.** Matched-only (cleanest, but a loss would not survive "was the control
given a chance?"); 1e-3-only (invites "would tiny also improve at another rate?", which we do not
test). A learning-rate sweep for either model — out of scope; the tiny recipe is the published one.

**Consequence.** One extra run of a 1.965 M-parameter network, the cheapest training job in the
project. If the 1e-3 control beats tiny-TL, that is the finding, not an embarrassment.

---

## 2026-09-17 — D026: Cached teacher logits live outside git, under `artifacts/logits/<split>_<protocol>/`

**Context.** ROADMAP §6 M0 says "300M test-split logits cached fp16 on Drive with a manifest."
There is no Drive now (D021). Each 512×512×2 fp16 logit map is 1 MB; the test split is ~90 MB
and the train split will be ~250 MB — over the 10 MB artifact limit, so they cannot be committed.

**Decision.** Logits are written on the studio to `artifacts/logits/test_native512/` (gitignored
via `/artifacts/`), one `.npy` per chip plus `manifest.json` carrying per-file SHA-256, the
checkpoint provenance (Hub revision, epoch), the protocol, and the environment. **Only the
manifest is copied back and committed**, as `results/runs/logits_test_native512_manifest.json`,
so every downstream use can verify the shards it consumed. `DATA.md` records the studio path.

**Consequence.** Distillation (M2) runs on the studio where the shards are; the manifest is the
audit trail. If the studio is lost, the shards are regenerated by the same command and must
match the committed checksums.
