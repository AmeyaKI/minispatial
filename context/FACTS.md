...
| **tiny-TL + UperNetDecoder(256) + LearnedInterpolateToPyramidal neck = 13.0 M total trainable parameters** (Lightning model summary, smoke run 2026-09-15). The decoder and neck add ~7.4 M on top of the 5.63 M backbone; the *deployed* "5M model" is a 13 M model. | `verified` | `terratorch fit` model summary on the Mac, `train/configs/tiny_tl.yaml` | 2026-09-15 |
| 100M-TL is **86.24 M** backbone parameters, 12 blocks, embed dim 768; tiny-TL is 5.63 M, 12 blocks, embed 192. Both take a 512×512 input (1025 tokens) as well as 224. The "100M" is a family name, not a count. | `verified` | built both from `BACKBONE_REGISTRY` locally and counted parameters; forward at 224 and 512 | 2026-09-15 |
