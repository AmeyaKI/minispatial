...
def build(commit: str, repo_url: str) -> dict:
    cells = [
        ...
        _code(
            ...
            "txts = list(Path(DATA_ROOT).rglob('*.txt'))\n"
            "print(len(tifs), 'tif files,', len(csvs), 'split csvs,', len(txts), 'split txts')\n"
            ...
            "# terratorch reads .txt, not the .csv the bucket ships; the download step\n"
            "# derives them. Without these, the datamodule dies on FileNotFoundError.\n"
            "assert len(txts) == 4, f'expected 4 derived split txts, got {len(txts)}'\n"
